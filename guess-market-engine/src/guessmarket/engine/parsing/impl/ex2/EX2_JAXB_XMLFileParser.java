// Ex2's JAXB-based parser. Superseded by Ex3's schema (no `GM-users`, no per-event
// `id` - events are identified by `name`, users come from login instead of XML),
// so this no longer compiles as-is.
